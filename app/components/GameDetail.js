...
export default function GameDetail({ title }) {
  ...
  return (
    <div className="p-4 border border-gray-200 rounded-md shadow-sm mx-auto w-96 bg-gray-800">
      ...
      {game.image && <img src={game.image} alt={game.name} className="mx-auto mb-4 w-64 h-64 object-contain" />}
      <p className="text-gray-700 mb-2 text-center">Cheapest price: ${game.cheapest}</p>
    </div>
  );
}